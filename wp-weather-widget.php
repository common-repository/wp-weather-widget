...
Plugin Name: WP Weather Widget

Plugin URI: http://www.teamwebusa.com/wp-weather-widget

Description: Show local weather in the sidebar of your wordpress theme. Shows 1 day to 5 day forecasts.

Author: Max Oaktree

Author URI: http://www.websitedesignersindallas.net/
...
wp_weather_widget_cache 				    = How many seconds to cache weather: default 3600 (one hour).

wp_weather_widget_error 					= Error message if weather is not found.

wp_weather_widget_sizes 					= array of sizes for widget

wp_weather_widget_extended_forecast_text 	= Change text of footer link
...
add to your URL 'clear_wp_weather_widget' 

For example: http://url.com/?clear_wp_weather_widget
...
$wp_weather_widget_sizes = apply_filters( 'wp_weather_widget_sizes' , array( 'tall', 'wide' ) );
...
function wp_weather_widget_setup()

{

	load_plugin_textdomain( 'wp-weather-widget', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );

}

add_action('plugins_loaded', 'wp_weather_widget_setup', 99999);
...
function wp_weather_widget_wp_head( $posts ) 

{

	wp_enqueue_style( 'wp-weather-widget', plugins_url( '/wp-weather-widget.css', __FILE__ ) );

	wp_enqueue_style( 'opensans-googlefont', 'https://fonts.googleapis.com/css?family=Open+Sans:400,300' );

}

add_action('wp_enqueue_scripts', 'wp_weather_widget_wp_head');
...
add_shortcode( 'wp-weather-widget', 'wp_weather_widget_shortcode' );

function wp_weather_widget_shortcode( $atts )

{

	return wp_weather_widget_logic( $atts );	
...
function wp_weather_widget_logic( $atts )

{

	global $wp_weather_widget_sizes;
	...
	$units_display		= $units == "metric" ? __('C', 'awesome-weather') : __('F', 'wp-weather-widget');
	...
	if( !$location ) { return wp_weather_widget_error(); }
    ...
    if( isset($_GET['clear_wp_weather_widget']) )
...

	{
...

		{
			...
			return wp_weather_widget_error( $now_ping_get->get_error_message()  ); 
...

		{
			...
			return wp_weather_widget_error( $city_data->message ); 
...

		{
...

			{
				...
				return wp_weather_widget_error( $forecast_ping_get->get_error_message()  ); 
...

			{
				...
				return wp_weather_widget_error( $forecast_data->message ); 
...

		{
			...
			set_transient( $weather_transient_name, $weather_data, apply_filters( 'wp_weather_widget_cache', 11000 ) ); 
	...
	if( !$weather_data OR !isset($weather_data['now'])) { return wp_weather_widget_error(); }
	...
	$wind_label = array ( 
							...
							__('N', 'wp_weather_widget'),

							__('NNE', 'wp_weather_widget'), 

							__('NE', 'wp_weather_widget'),

							__('ENE', 'wp_weather_widget'),

							__('E', 'wp_weather_widget'),

							__('ESE', 'wp_weather_widget'),

							__('SE', 'wp_weather_widget'),

							__('SSE', 'wp_weather_widget'),

							__('S', 'wp_weather_widget'),

							__('SSW', 'wp_weather_widget'),

							__('SW', 'wp_weather_widget'),

							__('WSW', 'wp_weather_widget'),

							__('W', 'wp_weather_widget'),

							__('WNW', 'wp_weather_widget'),

							__('NW', 'wp_weather_widget'),

							__('NNW', 'wp_weather_widget')
	...
	$show_stats_class = ($show_stats) ? "wpw_with_stats" : "wpw_without_stats";
	...
	$rtn .= "
		...
		<div id=\"wp-weather-widget-{$city_name_slug}\" class=\"wp-weather-widget-wrap wpwcf {$bg_color} {$show_stats_class} wpw_{$size}\"{$inline_style}>
	...
	{ 
		...
		$rtn .= "<div class=\"wp-weather-widget-cover\" style='background-image: url($background);'>";

		$rtn .= "<div class=\"wp-weather-widget-darken\">";
	...
	$rtn .= "
			...
			<div class=\"wp-weather-widget-header\">{$header_title}</div>

			

			<div class=\"wp-weather-widget-current-temp\">

				$today_temp<sup>{$units_display}</sup>

			</div> <!-- /.wp-weather-widget-current-temp -->
...

	{
		...
		$speed_text = ($units == "metric") ? __('km/h', 'wp-weather-widget') : __('mph', 'wp-weather-widget');
		...
		$rtn .= "
				...
				<div class=\"wp-weather-widget-todays-stats\">

					<div class=\"wpw_desc\">{$today->weather[0]->description}</div>

					<div class=\"wpw_humidty\">" . __('humidity:', 'wp-weather-widget') . " {$today->main->humidity}% </div>

					<div class=\"wpw_wind\">" . __('wind:', 'wp-weather-widget') . " {$today->wind->speed}" . $speed_text . " {$wind_direction}</div>

					<div class=\"wpw_highlow\"> "  .__('H', 'wp-weather-widget') . " {$today_high} &bull; " . __('L', 'wp-weather-widget') . " {$today_low} </div>	

				</div> <!-- /.wp-weather-widget-todays-stats -->
...

	{
		...
		$rtn .= "<div class=\"wp-weather-widget-forecast wpw_days_{$days_to_show} wpwcf\">";
...

		{
			...
			$days_of_week = array( __('Sun' ,'wp-weather-widget'), __('Mon' ,'wp-weather-widget'), __('Tue' ,'wp-weather-widget'), __('Wed' ,'wp-weather-widget'), __('Thu' ,'wp-weather-widget'), __('Fri' ,'wp-weather-widget'), __('Sat' ,'wp-weather-widget') );
			...
			$rtn .= "
				...
				<div class=\"wp-weather-widget-forecast-day\">

					<div class=\"wp-weather-widget-forecast-day-temp\">{$forecast->temp}<sup>{$units_display}</sup></div>

					<div class=\"wp-weather-widget-forecast-day-abbr\">$day_of_week</div>
		...
		$rtn .= " </div> <!-- /.wp-weather-widget-forecast -->";
...

	{
		...
		$show_link_text = apply_filters('wp_weather_widget_extended_forecast_text' , __('extended forecast', 'wp-weather-widget'));



		$rtn .= "<div class=\"wp-weather-widget-more-weather-link\">";

		$rtn .= "<a href=\"http://openweathermap.org/city/{$today->id}\" target=\"_blank\">{$show_link_text}</a>";		

		$rtn .= "</div> <!-- /.wp-weather-widget-more-weather-link -->";
	...
	{ 
		...
		$rtn .= "</div> <!-- /.wp-weather-widget-cover -->";

		$rtn .= "</div> <!-- /.wp-weather-widget-darken -->";

	}

	$rtn .= "<div class=\"designed\">Powered: <a href=\"http://www.teamwebusa.com/\">Web Design</a></div>";

	$rtn .= "</div> <!-- /.wp-weather-widget-wrap -->";
...
function wp_weather_widget_error( $msg = false )

{

	if(!$msg) $msg = __('No weather information available', 'wp-weather-widget');

	return apply_filters( 'wp_weather_widget_error', "<!-- WP WEATHER WIDGET ERROR: " . $msg . " -->" );
...
// WP WEATHER WIDGET, WIDGET CLASS

class WpWeatherWidget extends WP_Widget 

{

	function WpWeatherWidget() { parent::WP_Widget(false, $name = 'WP Weather Widget'); }
    ...
    {	
		...
		echo wp_weather_widget_logic( array( 'location' => $location, 'override_title' => $override_title, 'size' => $size, 'units' => $units, 'forecast_days' => $forecast_days, 'hide_stats' => $hide_stats, 'show_link' => $show_link, 'background' => $background, 'custom_bg_color' => $custom_bg_color ));
	...
	?>
        ...
        <p>
          ...
          <label for="<?php echo $this->get_field_id('location'); ?>">
          	...
          	<?php _e('Location:', 'wp-weather-widget'); ?><br />

          	<small><?php _e('(i.e: London,UK or New York City,NY)', 'wp-weather-widget'); ?></small>
        ...
        <p>
          ...
          <label for="<?php echo $this->get_field_id('override_title'); ?>"><?php _e('Override Title:', 'wp-weather-widget'); ?></label> 
        ...
        <p>
          ...
          <label for="<?php echo $this->get_field_id('units'); ?>"><?php _e('Units:', 'wp-weather-widget'); ?></label>  &nbsp;
		...
		<p>
          ...
          <label for="<?php echo $this->get_field_id('size'); ?>"><?php _e('Size:', 'wp-weather-widget'); ?></label> 

          <select class="widefat" id="<?php echo $this->get_field_id('size'); ?>" name="<?php echo $this->get_field_name('size'); ?>">

          	<?php foreach($wp_weather_widget_sizes as $size) { ?>
		...
		<p>
          ...
          <label for="<?php echo $this->get_field_id('forecast_days'); ?>"><?php _e('Forecast:', 'wp-weather-widget'); ?></label> 
        ...
        <p>
          ...
          <label for="<?php echo $this->get_field_id('background'); ?>"><?php _e('Background Image:', 'wp-weather-widget'); ?></label> 
        ...
        <p>
          ...
          <label for="<?php echo $this->get_field_id('custom_bg_color'); ?>"><?php _e('Custom Background Color:', 'wp-weather-widget'); ?></label><br />

          <small><?php _e('overrides color changing', 'wp-weather-widget'); ?>: #7fb761 or rgba(0,0,0,0.5)</small>
        ...
        <p>
          ...
          <label for="<?php echo $this->get_field_id('hide_stats'); ?>"><?php _e('Hide Stats:', 'wp-weather-widget'); ?></label>  &nbsp;
        ...
        <p>
          ...
          <label for="<?php echo $this->get_field_id('show_link'); ?>"><?php _e('Link to OpenWeatherMap:', 'wp-weather-widget'); ?></label>  &nbsp;
		...
		<p>  
          ...
          <label for="<?php echo $this->get_field_id('show_designed_link'); ?>"><?php _e('Link to Designed By:', 'wp-weather-widget'); ?></label>  &nbsp;

		  

          <input id="<?php echo $this->get_field_id('show_designed_link'); ?>" name="<?php echo $this->get_field_name('show_designed_link'); ?>" type="checkbox" value="1" <?php if($show_designed_link) echo ' checked="checked"'; ?> />
        ...
        <p>
          ...
          <label for="<?php echo $this->get_field_id('widget_title'); ?>"><?php _e('Widget Title: (optional)', 'wp-weather-widget'); ?></label> 
...
add_action( 'widgets_init', create_function('', 'return register_widget("WpWeatherWidget");') );